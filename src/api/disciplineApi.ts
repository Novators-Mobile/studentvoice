...
export const getDisciplines = async ({ search, teacherId, universityId }: TDisciplineParams): Promise<TDiscipline[]> => {
  const response = await axiosInstance.get<TDiscipline[]>("/subject/", { params: {
    search: search,
    teacher: teacherId,
...
export const postDiscipline = async (data: TDiscipline): Promise<TDiscipline> => {
